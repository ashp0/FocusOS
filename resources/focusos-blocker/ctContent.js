...
function blockPage(version, param, reason) {
	...
	if (!pageBlocked) {
		...
		/* FocusOS: render a LOCAL blocked page instead of the remote
		   getcoldturkey.com/blocked iframe. No network is required (the session
		   may have the network clamped) and there is no break/unblock bypass UI. */
		iframe.src = chrome.runtime.getURL('blocked.html')
			+ (reason && reason != "" ? ('?reason=' + unicode_to_b64url(JSON.stringify(reason))) : '');
